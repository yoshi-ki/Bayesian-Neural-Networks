...
class BayesLinear_Normalq(nn.Module):
    ...
    def __init__(self, n_in, n_out, prior_class):
        ...
        self.W_mu = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.n_in, self.n_out)))
        #self.W_p = nn.Parameter(torch.Tensor(self.n_in, self.n_out).uniform_(-3, -2))
        # self.W_p = nn.Parameter(torch.Tensor(self.n_in, self.n_out).uniform_(-3, -2))
        self.W_p = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.n_in,self.n_out)))

        # self.b_mu = nn.Parameter(torch.Tensor(self.n_out).uniform_(-0.1, 0.1))
        # self.b_mu = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.n_out)))
        self.b_mu = nn.Parameter(torch.Tensor(self.n_out).uniform_( -np.sqrt(6/(1+self.n_out)), np.sqrt(6/(1+self.n_out)) ))
        #self.b_p = nn.Parameter(torch.Tensor(self.n_out).uniform_(-3, -2))
        # self.b_p = nn.Parameter(torch.Tensor(self.n_out).uniform_(-3, -2))
        # self.b_p = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.n_out)))
        self.b_p = nn.Parameter(torch.Tensor(self.n_out).uniform_( -np.sqrt(6/(1+self.n_out)), np.sqrt(6/(1+self.n_out)) ))
...
class BayesConv_Normalq(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, prior_class, stride=1, padding=0):
        ...
        self.W_mu = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.out_channels,self.in_channels,self.kernel_size,self.kernel_size)))
        # self.W_p = nn.Parameter(torch.Tensor(self.out_channels, self.in_channels, self.kernel_size, self.kernel_size).uniform_(-2, 0))
        self.W_p = nn.Parameter(nn.init.xavier_normal_(torch.empty(self.out_channels, self.in_channels, self.kernel_size, self.kernel_size)))

        # self.b_mu = nn.Parameter(torch.Tensor(self.out_channels).uniform_(-0.1, 0.1))
        self.b_mu = nn.Parameter(torch.Tensor(self.out_channels).uniform_( -np.sqrt(6/(1+self.out_channels)), np.sqrt(6/(1+self.out_channels)) ))
        # self.b_p = nn.Parameter(torch.Tensor(self.out_channels).uniform_(-2, 0))
        self.b_p = nn.Parameter(torch.Tensor(self.out_channels).uniform_( -np.sqrt(6/(1+self.out_channels)), np.sqrt(6/(1+self.out_channels)) ))
    ...
    def forward(self, X, sample=False):
        ...
        if not self.training and not sample:  # When training return MLE of w for quick validation
            # output = torch.mm(X, self.W_mu) + self.b_mu.expand(X.size()[0], self.n_out)
            output = F.conv2d(X, self.W_mu, self.b_mu)
            return output, 0, 0
...
class bayes_conv_2L(nn.Module):
    ...
    def __init__(self, input_dim, in_channels, output_dim, prior_instance):
        ...
        self.conv1 = BayesConv_Normalq(self.in_channels,32,3,self.prior_instance)
        self.conv2 = BayesConv_Normalq(32,64,3,self.prior_instance)
        self.pool = nn.MaxPool2d(2,2)
        self.bfc1 = BayesLinear_Normalq(9216, 128, self.prior_instance)
        self.bfc2 = BayesLinear_Normalq(128, self.output_dim, self.prior_instance)
...
class BBP_Bayes_Conv_Net(BaseNet):
    ...
    def __init__(self, lr=1e-3, channels_in=3, side_in=28, cuda=True, classes=10, batch_size=128, Nbatches=0, prior_instance=laplace_prior(mu=0, b=0.1)):
        super(BBP_Bayes_Conv_Net, self).__init__()
    ...
    def create_net(self):
        ...
        self.model = bayes_conv_2L(input_dim=self.channels_in * self.side_in * self.side_in,in_channels=self.channels_in, output_dim=self.classes, prior_instance=self.prior_instance)
        if self.cuda:
            self.model.cuda()
