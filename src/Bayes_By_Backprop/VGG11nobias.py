...
class BayesConv_Normalq(nn.Module):
    ...
    def forward(self, X, sample=False):
        ...
        if not self.training and not sample:  # When training return MLE of w for quick validation
            ...
            # output = torch.mm(X, W) + b.unsqueeze(0).expand(X.shape[0], -1)  # (batch_size, n_output)
            output = F.conv2d(X, W, padding=self.padding)
...
class bayes_VGG11(nn.Module):
    ...
    def forward(self, x, sample=False):
        ...
        x = self.pool2(x)
        # -----------------
        x, klw, klb = self.conv3(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x, klw, klb = self.conv4(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x = self.pool3(x)
        # -----------------
        x, klw, klb = self.conv5(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x, klw, klb = self.conv6(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x = self.pool4(x)
        # -----------------
        x, klw, klb = self.conv7(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x, klw, klb = self.conv8(x, sample)
        tklw = tklw + klw
        tklb = tklb + klb
        x = self.act(x)
        x = self.pool5(x)
...
class BBP_Bayes_VGG11_Net(BaseNet):
    ...
    def create_net(self):
        ...
        self.model = bayes_VGG11(input_dim=self.channels_in * self.side_in * self.side_in,in_channels=self.channels_in, output_dim=self.classes, prior_instance=self.prior_instance)
        if self.cuda:
            self.model.cuda()
